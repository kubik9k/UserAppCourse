...
internal class SignUpUser {
    ...
    suspend fun signUp(user: UserDataModel, successful: (Boolean) -> Unit) {
        withContext(Dispatchers.IO) {
            val docRef = FirebaseFirestore.getInstance().collection(Constants.USER_COLLECTION_NAME)
                .document(user.login)
            docRef.set(getUserMap(user = user)).addOnSuccessListener {
                successful(true)
            }.addOnFailureListener {
                Log.d("MyLog", "SignUpUser 'data' : ${it.message}")
                successful(false)
            }
        }
    }
